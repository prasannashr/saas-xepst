...
------------------------------------------------------------------------*/
angular.module('app').controller('projectdetailCtrl', function(Auth, $scope, $http, $routeParams, $stateParams, mvIdentity, $location, $log, $timeout, $filter, $translate, mvAuth, Notification, $rootScope, generalServices, Upload, $state) {
    alert(Auth);
    ...
    $http.get("/api/getSrStatus/" + $scope.identity.currentUser.customerId).success(function(result) {
        angular.forEach(result, function(value, key) {
            var status = {
                ...
                "code": value.setting.sr_status.code,
            ...
            $scope.checkedStatus.push(status._id);
            $scope.checkedStatus_.push(status._id);
        });
    ...
    $scope.showStatus = function() {
       ...
       /* if ($scope.selected[0] != undefined) {
        ...
        //$scope.resolutionType = false;
        return ($scope.srStatus && $scope.selected.length) ? $scope.selected[0].code : 'Not set';
    };

    /*----------------------------------------------------------------------------------------------------
        Name: getStatusSubCode
        Description: function to get all Resolution Type from the database 
        Author: Prasanna Shrestha
    -----------------------------------------------------------------------------------------------------*/
    $scope.getStatusSubCode = function(customerId, srStatus) {
              
        $http.get("/api/statusSubCode/" + customerId+"/"+srStatus).success(function(result) {   
           
            if(result.length>0){
                $scope.statusSubCode = true;
                $scope.statusSubCodes = result;
            }else{
                $scope.statusSubCode = false;
            }
           
        })
    }

    $scope.showStatusSubCode = function() {
        //$scope.statuses = ['New', 'Open', 'Waiting for Info', 'On Hold', 'In Progress', 'Resolved'];        
        $scope.selectedStatusSubCode = $filter('filter')($scope.statusSubCodes, {
            id: $scope.statusSubCodeId
        });
        return ($scope.statusSubCodeId && $scope.selectedStatusSubCode.length) ? $scope.selectedStatusSubCode[0].subCode : 'Not set';
    };
        ...
        if ($scope.form.$valid) {
            ...
            angular.forEach(files, function(file, key) {
                file.upload = Upload.upload({
                    url: 'api/uploadSingleAttachment?projectId=' + projectId,
                    data: {
                        file: file
                    }
                });
    ...
    **/
    ...
    $scope.showSRDetails = function(srId) {
        ...
        $http.get("/api/getSrDetail/" + srId).success(function(srData) {
            ...
            if ($scope.selectedId) {
                ...
                $scope.getStatusSubCode($scope.identity.currentUser.customerId, $scope.srStatus);
                $scope.assignee = $scope.selectedId.assignee;
                $scope.statusSubCodeId = $scope.selectedId.resolution_type;
                $scope.getRelatedStatus($scope.srStatus);
    ...
    $scope.addComment = function(comment, srId) {
        ...
        if (comment != null && comment != '') {
            ...
            $scope.error = $filter('translate')('REQUIRED_MESSAGE');
    ...
    $scope.updateSrComment = function(comment, commentId, srId) {
        ...
        $http.put("/api/updateSrComment/" + commentId, dataObject).success(function(data) {
            ...
            $scope.showSRDetails(srId);
            if (!data.deleted) {
                Notification.info($filter('translate')('COMMENT_UPDATE_SUCCESS_MESSAGE'));
            } else if (data.deleted) {
                Notification.info($filter('translate')('COMMENT_DELETE_SUCCESS_MESSAGE'));
            }
        }).error(function(data, status) {
            console.log(data);
            $scope.errMessage = $filter('translate')('ERROR_MESSAGE');
    ...
    $scope.getRelatedStatus = function(srStatusId){
       
        mvAuth.getStatusRelatedStatus(srStatusId).then(function(response) {
    ...
    $scope.passValue = function(value) {
        ...
        $scope.updatePriority = function(srId) {
            $http.put('api/updatePriority/' + srId, {
                ...
                Notification.info($filter('translate')('PRIORITY_UPDATE_SUCCESS_MESSAGE'));
            }).error(function(data, status) {
                console.log(data);
                $scope.errMessage = $filter('translate')('ERROR_MESSAGE');
                Notification.error($scope.errMessage);
    ...
    $scope.uploadSingleFile = function(attach) {
        ...
        file.upload = Upload.upload({
            url: 'api/uploadSingleAttachment?projectId=' + projectId + '&srId=' + $scope.selectSR,
            data: {
                file: file,
    ...
    $scope.checkAllIncomplete = function () {
        angular.forEach($scope.sr_statuses, function (s) {
            if(s.order < 6000) {
                ...
                if ($scope.checkedStatus.indexOf(s._id) == -1) {
                    if($scope.allIncomplete) {
                        $scope.checkedStatus.push(s._id);
                    }
                } else {
                    if ($scope.checkedStatus.indexOf(s._id) !== -1) {
                        if(!$scope.allIncomplete) {
                            index = $scope.checkedStatus.indexOf(s._id);
