...
var fetchCustomerDetails = function(user, done) {
    ...
    Customer.findOne({
        ...
        if (err) {
            ...
            if (customer) {
                ...
                if(user.logo!==undefined && user.logo!==null && user.logo!=="") {
                    //gets image data by using _id of document in thumbnails.files collection
                    fileHandler.getImageThumbnail(user.logo, function(err, image) {
                        if(err) {
                            return done(err);
                        }
                        user.orgLogoImage = image;
    ...
    passport.deserializeUser(function(id, done) {
        ...
        User.aggregate({
            ...
            if (user) {
                ...
                if(user.app_users.proPic!=undefined && user.app_users.proPic!=null && user.app_users.proPic!="") {                    
                    //gets image data by using _id of document in thumbnails.files collection
                    fileHandler.getImageThumbnail(user.app_users.proPic, function(err, image) {
                        if(err) {
                            return done(err);
                        }
                        user.app_users['profileImage'] = image;
                        fetchCustomerDetails(user, function(err, user) {
                            if(err) {
                                return done(err);
                            }
                            return done(null, user);
